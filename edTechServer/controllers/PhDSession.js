const PhDSession = require("../models/PhDSession");

// Create a new PhD session
exports.createSession = async (req, res) => {
  try {
    ...
    console.error("PhD createSession error", err);
...
exports.getSessions = async (_req, res) => {
  try {
    const list = await PhDSession.find({}).sort({ createdAt: -1 });
    return res.json({ success: true, data: list });
  } catch (err) {
    console.error("PhD getSessions error", err);
...
exports.getSessionById = async (req, res) => {
  try {
    const { id } = req.params;
    const doc = await PhDSession.findById(id);
    if (!doc) return res.status(404).json({ success: false, message: "Session not found" });
    return res.json({ success: true, data: doc });
  } catch (err) {
    console.error("PhD getSessionById error", err);
...
exports.updateSession = async (req, res) => {
  try {
    ...
    if (update.name) {
      const exists = await PhDSession.findOne({ name: update.name, _id: { $ne: id } });
      if (exists) return res.status(409).json({ success: false, message: "A session with this name already exists" });
    }

    const doc = await PhDSession.findByIdAndUpdate(id, update, { new: true });
    if (!doc) return res.status(404).json({ success: false, message: "Session not found" });
    return res.json({ success: true, data: doc });
  } catch (err) {
    console.error("PhD updateSession error", err);
...
exports.deleteSession = async (req, res) => {
  try {
    const { id } = req.params;
    const doc = await PhDSession.findByIdAndDelete(id);
    if (!doc) return res.status(404).json({ success: false, message: "Session not found" });
    return res.json({ success: true, message: "Session deleted" });
  } catch (err) {
    console.error("PhD deleteSession error", err);
