...
export default function CourseTypes() {
  ...
  // Fetch schools and courses from backend
  ...
  const fetchCourses = async () => {
    ...
    setError("");
    try {
      const res = await apiConnector("GET", "/api/v1/ugpg-course");
      const list = res?.data?.data || [];
        ...
        <div style={{ width: "100%", overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: 0 }}>
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={9} style={{ padding: 16 }}>Loading...</td>
                </tr>
              ) : paged.length === 0 ? (
                <tr>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.25)", display: "flex", alignItems: "center", justifyContent: "center", paddingTop: "10vh", zIndex: 50 , marginLeft:"100px" }}>
          <div style={{ width: "min(980px, 95vw)", background: "#fff", borderRadius: 12, boxShadow: "0 10px 30px rgba(0,0,0,0.2)" }}>
            <div style={{ background: "#f8caff", padding: 16, borderTopLeftRadius: 12, borderTopRightRadius: 12, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div style={{ fontWeight: 700, fontSize: 18 }}>Add New Course</div>
            ...
            <div style={{ padding: 20 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16 }}>
                <div>
                  ...
                  <select name="school" value={form.school} onChange={onChange} style={{ width: "100%", padding: 10, border: "1px solid #cbd5e1", borderRadius: 8 }}>
              ...
              <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 24 }}>
                ...
                <button
                  disabled={submitting || !form.school || !form.category || !form.courseName}
                  onClick={async () => {
                    setSubmitError("");
                    if (!form.school || !form.category || !form.courseName.trim()) {
                      setSubmitError("School, Category and Course Name are required");
                      return;
                    }
                    setSubmitting(true);
                    try {
                      await apiConnector("POST", "/api/v1/ugpg-course", {
                        school: form.school,
