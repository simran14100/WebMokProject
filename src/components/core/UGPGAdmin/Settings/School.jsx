...
export default function UGPGSettingsSchool() {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((d) =>
      [d?.name, d?.status].some((v) => (v || "").toString().toLowerCase().includes(q))
    );
  ...
  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12, marginTop: "12rem" }}>
    <h2 style={{ fontSize: 18, fontWeight: 700, color: "#0A2F5A" }}>Manage Schools</h2>
    <button 
      onClick={() => { setForm({ title: "", shortcode: "" }); setSubmitError(""); setOpen(true); }} 
      style={{ background: "#0A2F5A", color: "#fff", border: 0, borderRadius: 8, padding: "8px 12px", fontWeight: 600 }}
    >
      + ADD NEW
    </button>
  </div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead style={{ background: "#E6F6FB" }}>
            <tr>
              ...
              <th />
              <th style={{ textAlign: "left", padding: 12, borderBottom: "1px solid #eaeef3", color: "#0A2F5A" }}>Title</th>
              <th style={{ textAlign: "left", padding: 12, borderBottom: "1px solid #eaeef3", color: "#0A2F5A" }}>Shortcode</th>
  ...
  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 12, color: "#334155" }}>
    <div>Showing {Math.min((page - 1) * limit + 1, total)} to {Math.min(page * limit, total)} of {total} entries</div>
    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
      <button 
        onClick={onPrev} 
        disabled={page <= 1} 
        style={{ padding: "6px 10px", border: "1px solid #e2e8f0", borderRadius: 8, background: page <= 1 ? "#f1f5f9" : "#00B4EB", color: page <= 1 ? "#334155" : "#fff" }}
      >
        Previous
      </button>
      ...
      <button 
        ...
        style={{ padding: "6px 10px", border: "1px solid #e2e8f0", borderRadius: 8, background: page >= totalPages ? "#f1f5f9" : "#00B4EB", color: page >= totalPages ? "#334155" : "#fff" }}
      >
        Next
      </button>
    </div>
  ...
  {open && (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.25)", display: "flex", alignItems: "flex-start", justifyContent: "center", paddingTop: "10vh", zIndex: 50  , marginTop:"6rem"}}>
      <div style={{ width: "min(720px, 95vw)", background: "#fff", borderRadius: 12, boxShadow: "0 10px 30px rgba(0,0,0,0.2)" }}>
        <div style={{ background: "#00B4EB", padding: 16, borderTopLeftRadius: 12, borderTopRightRadius: 12, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ fontWeight: 700, fontSize: 18, color: "#fff" }}>Add New School</div>
        ...
        <div style={{ padding: 20 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            <div>
              <label style={{ display: "block", fontSize: 14, marginBottom: 6, color: "#0A2F5A" }}>Title</label>
              <input
                value={form.title}
            ...
            <div>
              <label style={{ display: "block", fontSize: 14, marginBottom: 6, color: "#0A2F5A" }}>Short Code</label>
              <input
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  setSubmitError(e?.response?.data?.message || e.message || "Failed to create school");
                } finally {
                  setSubmitting(false);
                }
              }}
              style={{ background: "#0A2F5A", color: "#fff", border: 0, borderRadius: 8, padding: "8px 16px", fontWeight: 600 }}
            >
