  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 , marginTop: "12rem" }}>
        <h2 style={{ fontSize: 18, fontWeight: 700 }}>Manage Schools</h2>
        <button onClick={() => { setForm({ title: "", shortcode: "" }); setSubmitError(""); setOpen(true); }} style={{ background: "#6d28d9", color: "#fff", border: 0, borderRadius: 8, padding: "8px 12px", fontWeight: 600 }}>+ ADD NEW</button>
      </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead style={{ background: "#f8fafc" }}>
                <tr>
                  ...
                  <th style={{ textAlign: "left", padding: 12, borderBottom: "1px solid #eaeef3" }}>Title</th>
                  <th style={{ textAlign: "left", padding: 12, borderBottom: "1px solid #eaeef3" }}>Shortcode</th>
                ...
                {loading ? (
                  ...
                  paged.map((row) => (
                    <tr key={row._id}>
                      ...
                      <td style={{ padding: 12, borderBottom: "1px solid #f1f5f9" }}>{row?.shortcode || ""}</td>
                      <td style={{ padding: 12, borderBottom: "1px solid #f1f5f9" }}>{row?.status || "-"}</td>
                    </tr>
                  ))
                )}
              </tbody>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.25)", display: "flex", alignItems: "flex-start", justifyContent: "center", paddingTop: "10vh", zIndex: 50 }}>
          <div style={{ width: "min(720px, 95vw)", background: "#fff", borderRadius: 12, boxShadow: "0 10px 30px rgba(0,0,0,0.2)" }}>
            <div style={{ background: "#f8caff", padding: 16, borderTopLeftRadius: 12, borderTopRightRadius: 12, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div style={{ fontWeight: 700, fontSize: 18 }}>Add New School</div>
            ...
            <div style={{ padding: 20 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                ...
                <div>
                  <label style={{ display: "block", fontSize: 14, marginBottom: 6 }}>Short Code</label>
                  <input
